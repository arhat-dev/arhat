...
import (
	...
	"arhat.dev/arhat/pkg/connectivity/connectivityutils"
	"arhat.dev/arhat/pkg/types"
)

func init() {
	connectivity.Register("serial", aranyagopb.CONNECTIVITY_MODE_CLIENT, NewSerialDeviceConnectivity)
}
...
var _ types.Connectivity = &serialDeviceConnectivity{}
...
func (s *serialDeviceConnectivity) doReadAfterWrite(params map[string]string) ([][]byte, error) {
	...
	return connectivityutils.ReadAfterWrite(s.openedPort, s.openedPort, params)
}

func (s *serialDeviceConnectivity) Operate(params map[string]string, data []byte) ([][]byte, error) {
	return s.doReadAfterWrite(params)
}

func (s *serialDeviceConnectivity) CollectMetrics(params map[string]string) ([]*types.DeviceMetricValue, error) {
	//return s.doReadAfterWrite(params)
	return nil, nil
}
